...
-- start evaluate
--
aDP,aGP,aTP = 0,0,0  -- detected/groundtruth/true positive
-- loop
for oindex = 36,50 do
   ----------------------------------------------------------------------
   -- load image
   ...
   _,coordinate = loadLabels(oindex,geometry)
   ...
   if opt.demo then
      -- true positive
      if TP ~= 0 then
         getBoxedImages(dataO,grR[correctLabel:eq(1)],grC[correctLabel:eq(1)],{0.2,0.8,0.4},94,10)
      end
      -- false positive
      if DP ~= TP then
         getBoxedImages(dataO,grR[correctLabel:eq(0)],grC[correctLabel:eq(0)],{0.2,0.4,0.8},94,10)
      end
      -- false negative
      if GP ~= TP then
         ...
         getBoxedImages(dataO,tgR,tgC,{0.92,0.92,0.15},94,10)
      end
      -- save output
      image.save('output/F'..oindex..'.png',(dataO*255):byte())
   end
...
print('<result> Precision: '..cg..string.format('%.3f%%',prcision*100))
